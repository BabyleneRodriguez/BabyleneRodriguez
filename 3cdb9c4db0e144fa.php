...
# /3cdb9c4db0e144fa.php: Javascript tracking code
# /597d41222138cad3.php: Beacon

error_reporting(0);
ob_start();

$cachefile = sys_get_temp_dir() . "/3cdb9c4db0e144fa.php.cache";
if( file_exists( $cachefile ) && filemtime( $cachefile ) > (time()-86400)) {
	$x = unserialize( file_get_contents( $cachefile ));
	if( is_array( $x )) $x['headers'][] = "x-cache-php: HIT";
	else unset( $x );
}

if( !$x ) {
	$x = proxyget( "https://static.getclicky.com/js?in=%2F597d41222138cad3.php" );
	$CACHE_SAVE = 1;
}
...
if( is_array( $x )) {
	...
	echo $x['content']; 
	if( $CACHE_SAVE && $x['code']==200 ) file_put_contents( $cachefile, serialize( $x ), LOCK_EX );
}
